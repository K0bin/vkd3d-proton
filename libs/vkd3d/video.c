...
HRESULT STDMETHODCALLTYPE d3d12_video_device_QueryInterface(d3d12_video_device_iface *iface,
        REFIID riid, void **object)
{
    ...
    {
        ID3D12VideoDevice_AddRef(iface);
        *object = iface;
        return S_OK;
    }
...
static ULONG STDMETHODCALLTYPE d3d12_video_device_AddRef(d3d12_video_device_iface *iface)
{
    struct d3d12_video_device *device = impl_from_ID3D12VideoDevice(iface);
...
HRESULT d3d12_create_video_device(ID3D12VideoDevice **video) {
    struct d3d12_video_device *video_device;
    if (!(video_device = vkd3d_malloc(sizeof(*video_device))))
        return E_OUTOFMEMORY;

    memset(video_device, 0, sizeof(*video_device));
    video_device->ID3D12VideoDevice_iface.lpVtbl = &d3d12_video_device_vtbl;
    video_device->refcount = 1;
